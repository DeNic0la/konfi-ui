...
import {BehaviorSubject, debounceTime, filter, map, Subject, Subscription} from "rxjs";
...
@Component({
  ...
  imports: [CommonModule, Card, FloatLabel, InputText, FormsModule, PushPipe, ReactiveFormsModule, KeyFilter, Password, Button],
...
export class BrunchCreateComponent implements OnInit{
  ...
  ngOnInit(): void {
  console.log("AA")
    const s = this.form.controls.title.valueChanges.pipe(
  ...
  public readonly form = this.fb.group({
    ...
    id: this.fb.control('',[Validators.required,Validators.minLength(3),Validators.pattern("^[a-zA-Z0-9-]+$"),Validators.maxLength(50)]),
  ...
  onSubmit() {
    const subscr = this.bcs.send(this.form.value).subscribe(value => console.log(value))
  this.sub.add(subscr)
  }
}
