...
@Component({
  ...
  template: `
    <div class="h-screen w-full" role="main" aria-labelledby="page-title">
      ...
      <p-toast key="sr-only" pStyleClass="sr-only" class="sr-only"
               role="alert" aria-live="polite">
      </p-toast>
...
export class TableComponent implements AfterViewInit {
  ...
  public joinedTable$ = this.username$.pipe(
    ...
    switchMap((name) => this.webSocketService.joinTable$(<string>this.id(),name)),
  ...
  public readonly connectionError$  = this.webSocketService.connectionStatus$.pipe(
    takeUntilDestroyed(),
