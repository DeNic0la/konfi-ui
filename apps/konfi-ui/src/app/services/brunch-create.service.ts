...
export class BrunchCreateService {
  ...
  const validData = brunch_schema.parse({title,id,adminPassword,votingPassword,questions: question})
...
const brunch_schema = z.object({
  title: z.string().nonempty(),
  id: z.string().min(3).max(50).regex(/^[a-zA-Z0-9-]+$/),
  ...
  questions: z.string().nonempty().transform(v=>([{title:v}])).pipe(z.array(question_schema))
})
